...
when 'debian'
  ...
  if platform?('debian') && Chef::VersionConstraint.new('>= 7.0').include?(node['platform_version']) ||
     platform?('ubuntu') && Chef::VersionConstraint.new('>= 12.04').include?(node['platform_version'])
