...
fetch_version()
{
  version=$(curl --insecure -s -B "${rvm_releases_url}/latest-version.txt" 2>/dev/null)
}

fetch_md5()
{
  md5=$(curl --insecure -s -B "${rvm_releases_url}/rvm-${version}.tar.gz.md5" 2>/dev/null)
}
...
install_release()
{
  ...
  if curl --insecure -L "${url}" -o "${archive}"
  then
    true
  else
    fail "Failed to download ${url} to ${archive} using 'curl --insecure', error code ($?)"
  fi
