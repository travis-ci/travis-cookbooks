...
if ! command -v curl --insecure > /dev/null ; then
  ...
  fetch_command="curl --insecure -x${rvm_proxy} -f -L --create-dirs -C - " # -s for silent

else

  fetch_command="curl --insecure -f -L --create-dirs -C - " # -s for silent
