action :create do
  unless exists?
    Chef::Log.info("Installing PHP #{@new_resource} with php-build")
    new_resource = @new_resource
    phpbuild_path = "#{node[:phpbuild][:home]}/.php-build"
    version = new_resource.version
    target_path = "#{new_resource.path}/#{version}"
    bash "install PHP with php-build" do
      ...
      EOF
      not_if "test -f #{target_path}/#{version}/bin/php"
    end
...
action :delete do
  if exists?
    Chef::Log.info("Uninstalling PHP #{@new_resource} from php-build")
    phpbuild_path = "#{node[:phpbuild][:home]}/.php-build"
