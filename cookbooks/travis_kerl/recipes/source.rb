...
# Copyright 2012-2015, Travis CI GmbH
#
...
node['travis_kerl']['releases'].each do |rel, _build|
  execute "build Erlang #{rel}" do
    ...
    not_if(
      "#{node['travis_kerl']['path']} list builds | grep \"#{rel}$\"",
      user: node['travis_build_environment']['user'],
      environment: env
    )
  end
  ...
  execute "install Erlang #{rel}" do
    ...
    not_if(
      "#{node['travis_kerl']['path']} list installations | grep #{rel}",
      user: node['travis_build_environment']['user'],
      environment: env
    )
  end
  ...
  execute "generate Erlang #{rel} Dialyzer PLT" do
    ...
    not_if(
      "#{node['travis_kerl']['path']} list installations | grep #{rel} && test -f #{installation_root}/#{rel}/dialyzer.plt",
      user: node['travis_build_environment']['user'],
      environment: env
    )
  end
